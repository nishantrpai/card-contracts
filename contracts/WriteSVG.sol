  ...
  function write(string memory text, string memory color, uint256 scale) view public returns (string memory) {
    ...
    uint256 cy = 0;
      ...
      uint256 width = LETTER_WIDTHS[normalized] != 0
			...
			letters = string(abi.encodePacked(letters,"<g transform='translate(",Strings.toString(letterPos),",",Strings.toString(cy),")'>",path,"</g>"));
			letterPos = letterPos + width + spacing;
		}
		
		string memory svg = string(abi.encodePacked("<g  transform='scale(", Strings.toString(scale) ,") fill='",color,"' fill-rule='evenodd' clip-rule='evenodd' aria-label='",text,"'>",letters,"</g>"));
		return svg;
  }
}